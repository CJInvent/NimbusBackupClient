...
func (a *App) refreshStorageIdentityLocked() controlplane.StorageStatus {
	...
	if err != nil {
		return controlplane.StorageStatus{Error: "storage state unavailable: " + err.Error()}
	}
	before := s.Snapshot()
	devices, discoveryErr := discoverStorageDevices()
	if err = s.Observe(devices, discoveryErr); err != nil {
		return controlplane.StorageStatus{Error: "storage state persistence failed: " + err.Error()}
	}
	after := s.Snapshot()
