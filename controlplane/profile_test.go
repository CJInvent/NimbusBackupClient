...
// ---------------------------------------------------------------------------
// The encryption answer a profile may carry (profile version 2).
//
// It exists to close the window in which an enrolled agent that has never
// reached its server REFUSES to back up, because it cannot tell "this org does
// not encrypt" from "I have not been told". Both answers must survive the trip
// intact, and a third value must be refused rather than quietly dropped —
// dropping it would leave the machine in exactly the unknown state the field
// exists to resolve, and blame nothing for it.

func TestProfileCarriesTheEncryptionAnswer(t *testing.T) {
	for _, want := range []string{EncryptionOn, EncryptionOff} {
		p, err := ParseProfile(validProfileJSON(t, func(m map[string]any) {
			m["encryption"] = want
		}))
		if err != nil {
			t.Fatalf("encryption=%q refused: %v", want, err)
		}
		if p.Encryption != want {
			t.Errorf("encryption = %q, want %q", p.Encryption, want)
		}
	}
}

// Absent is a real answer: "this profile does not say", which leaves the
// machine waiting for the server exactly as it did before the field existed.
// A profile generated by an older server must not become invalid.
func TestProfileEncryptionIsOptional(t *testing.T) {
	p, err := ParseProfile(validProfileJSON(t, func(m map[string]any) {
		delete(m, "encryption")
	}))
	if err != nil {
		t.Fatalf("a profile with no encryption key was refused: %v", err)
	}
	if p.Encryption != "" {
		t.Errorf("encryption = %q, want empty", p.Encryption)
	}
}

// THE ONE THAT MATTERS. A value outside the set is REFUSED, not ignored.
func TestProfileRefusesAnUnknownEncryptionValue(t *testing.T) {
	for _, bad := range []string{"maybe", "ON", "true", "enabled", "1"} {
		_, err := ParseProfile(validProfileJSON(t, func(m map[string]any) {
			m["encryption"] = bad
		}))
		if err == nil {
			t.Errorf("encryption=%q was accepted; an unrecognised answer must not "+
				"silently become 'the profile said nothing'", bad)
			continue
		}
		if !strings.Contains(err.Error(), "encryption") {
			t.Errorf("encryption=%q was refused for the wrong reason: %v", bad, err)
		}
	}
}

// The redaction line names it, because a support engineer reading a log needs
// to know which answer the machine was installed with — and still must not see
// the token.
func TestRedactedNamesTheEncryptionAnswerAndNeverTheToken(t *testing.T) {
	p, err := ParseProfile(validProfileJSON(t, func(m map[string]any) {
		m["encryption"] = EncryptionOff
	}))
	if err != nil {
		t.Fatalf("ParseProfile: %v", err)
	}
	red := p.Redacted()
	if !strings.Contains(red, "encryption=off") {
		t.Errorf("Redacted() does not name the encryption answer: %s", red)
	}
	if strings.Contains(red, "one-time-org-token") {
		t.Fatalf("Redacted() leaked the enrollment token: %s", red)
	}
}
